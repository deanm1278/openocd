/***************************************************************************
 *   Copyright (C) 2013 by Andrey Yurovsky, 2017 Deam Miller               *
...
#define SAMD_NVMCTRL_INTFLAG	0x10	/* NVM Interupt Flag Status & Clear */
#define SAMD_NVMCTRL_STATUS		0x12	/* NVM status register */
#define SAMD_NVMCTRL_ADDR		0x14	/* NVM address register */
#define SAMD_NVMCTRL_RUNLOCK	0x18	/* NVM Lock section register */

#define SAMD_CMDEX_KEY		0xA5UL
#define SAMD5_NVM_CMD(n)		((SAMD_CMDEX_KEY << 8) | (n & 0x7F))

/* NVMCTRL commands.  See section 25.8.2 in SAMD51 datasheet */
#define SAMD5_NVM_CMD_EP		0x00		/* Erase Page */
#define SAMD5_NVM_CMD_EB		0x01
#define SAMD5_NVM_CMD_WP		0x03		/* Write Page */
#define SAMD5_NVM_CMD_WQW	0x04		/* Write 128 bit word */
#define SAMD5_NVM_CMD_LR		0x11		/* Lock Region */
#define SAMD5_NVM_CMD_UR		0x12		/* Unlock Region */
#define SAMD5_NVM_CMD_SPRM	0x13		/* Set Power Reduction Mode */
#define SAMD5_NVM_CMD_CPRM	0x14		/* Clear Power Reduction Mode */
#define SAMD5_NVM_CMD_PBC	0x15		/* Page Buffer Clear */
#define SAMD5_NVM_CMD_SSB	0x16		/* Set Security Bit */

/* NVMCTRL bits */
#define SAMD_NVM_CTRLA_WMODE_MANUAL 0x30

/* Known identifiers */
#define SAMD_PROCESSOR_M4	0x06
#define SAMD_FAMILY_D5		0x00
...
/* Known SAMD51 parts. */
static const struct samd5_part samd51_parts[] = {
	{ 0x00, "SAMD51P20A", 1024, 256},
	{ 0x01, "SAMD51P19A", 512, 192},
	{ 0x02, "SAMD51N20A", 512, 256},
...
struct samd_family {
	...
	uint8_t family;
	uint8_t series;
	const struct samd5_part *parts;
	size_t num_parts;
};

/* Known SAMD families */
static const struct samd_family samd_families[] = {
	{ SAMD_PROCESSOR_M4, SAMD_FAMILY_D5, SAMD_SERIES_51,
...
static const struct samd5_part *samd_find_part(uint32_t id)
{
...
static int samd_protect_check(struct flash_bank *bank)
{
	...
	res = target_read_u16(bank->target,
			SAMD_NVMCTRL + SAMD_NVMCTRL_RUNLOCK, &lock);
	if (res != ERROR_OK)
		return res;
...
static int samd_check_error(struct target *target)
{
	...
	//TODO: figure out a way to check for NVM errors
	return ERROR_OK;
}
...
static int samd_issue_nvmctrl_command(struct target *target, uint16_t cmd)
{
	...
	res = target_write_u16(target,
			SAMD_NVMCTRL + SAMD_NVMCTRL_CTRLB, SAMD5_NVM_CMD(cmd));
	if (res != ERROR_OK)
		return res;
...
static int samd_erase_row(struct target *target, uint32_t address)
{
	...
	while(!(status & 0x01)){ //not ready
		...
		if(timeout == 0){
			LOG_ERROR("timeout waiting for NVM ready");
			return ret;
		}
	}
	...
	res = target_write_u32(target,
			SAMD_NVMCTRL + SAMD_NVMCTRL_ADDR, address);

	/* Issue the Erase Row command to erase that row. */
	if (res == ERROR_OK)
		res = samd_issue_nvmctrl_command(target,
				address == SAMD_USER_ROW ? SAMD5_NVM_CMD_EP : SAMD5_NVM_CMD_EB);
		...
		uint8_t startb, uint8_t endb)
{
	...
	res = target_read_u32(target, SAMD_NVMCTRL + SAMD_NVMCTRL_CTRLA, &nvm_ctrla);
	if (res == ERROR_OK)
		manual_wp = (nvm_ctrla & SAMD_NVM_CTRLA_WMODE_MANUAL) == 0;
	...
	if (manual_wp) {
		/* Trigger flash write */
		//TODO: check that WQW is correct command for this
		res = samd_issue_nvmctrl_command(target, SAMD5_NVM_CMD_WQW);
	} else {
	...
	for (prot_block = first_prot_bl; prot_block <= last_prot_bl; prot_block++) {
		if (set != bank->prot_blocks[prot_block].is_protected) {
			...
			res = samd_issue_nvmctrl_command(bank->target,
					set ? SAMD5_NVM_CMD_LR : SAMD5_NVM_CMD_UR);
			if (res != ERROR_OK)
				goto exit;
		}
	}
		...
		uint32_t offset, uint32_t count)
{
	...
	/* Check if we need to do manual page write commands */
	res = target_read_u32(bank->target, SAMD_NVMCTRL + SAMD_NVMCTRL_CTRLA, &nvm_ctrla);

	if (res != ERROR_OK)
		return res;

	if ( (nvm_ctrla & SAMD_NVM_CTRLA_WMODE_MANUAL) == 0)
		manual_wp = true;
	else
		manual_wp = false;

	res = samd_issue_nvmctrl_command(bank->target, SAMD5_NVM_CMD_PBC);
	if (res != ERROR_OK) {
	...
	while (count) {
		...
		if (manual_wp || pg_offset + 4 * nw < chip->page_size) {
			...
			res = samd_issue_nvmctrl_command(bank->target, SAMD5_NVM_CMD_WP);
...
COMMAND_HANDLER(samd_handle_set_security_command)
{
	...
	if (target) {
		...
		res = samd_issue_nvmctrl_command(target, SAMD5_NVM_CMD_SSB);
...
static const struct command_registration at91samd5_exec_command_handlers[] = {
	{
	...
	},
	{
		...
		.mode = COMMAND_EXEC,
		.help = "Print information about the current at91samd5 chip"
...
static const struct command_registration at91samd5_command_handlers[] = {
	{
		.name = "at91samd5",
		.mode = COMMAND_ANY,
		.help = "at91samd5 flash command group",
		.usage = "",
		.chain = at91samd5_exec_command_handlers,
	},
	COMMAND_REGISTRATION_DONE
};

struct flash_driver at91samd5_flash = {
	.name = "at91samd5",
	.commands = at91samd5_command_handlers,
