...
static int bfinplus_add_breakpoint(struct target *target, struct breakpoint *breakpoint)
{
  ...
  else /* BKPT_HARD */
  {
    ...
    {
      LOG_ERROR("%s: no more hardware breakpoint available for 0x%08lx",
    ...
    {
      LOG_WARNING("%s: target is locked, hardware breakpoint [0x%08lx] will be set when it's unlocked", target_name(target), breakpoint->address);
    }
    else
    {
...
static int bfinplus_remove_breakpoint(struct target *target, struct breakpoint *breakpoint)
{
  ...
  else /* BKPT_HARD */
  {
    ...
    if (i == BFINPLUS_MAX_HWBREAKPOINTS)
    {
      LOG_ERROR("%s: no hardware breakpoint at 0x%08lx",
        target_name(target), breakpoint->address);
      return ERROR_FAIL;
    }
...
static int bfinplus_add_watchpoint(struct target *target, struct watchpoint *watchpoint)
{
  ...
  if (bfinplus->is_locked)
  {
    LOG_WARNING("%s: target is locked, hardware watchpoint [0x%08lx] will be set when it's unlocked", target_name(target), watchpoint->address);
  }
  else
  {
...
static int bfinplus_remove_watchpoint(struct target *target, struct watchpoint *watchpoint)
{
  ...
  {
    LOG_ERROR("no hardware watchpoint at 0x%08lX length %d.",
