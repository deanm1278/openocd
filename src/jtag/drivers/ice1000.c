...
static int adi_connect(const uint16_t *vids, const uint16_t *pids)
{
	...
	{
		LOG_ERROR("malloc(%ld) fails", sizeof(dat_dat) * DAT_SZ);
...
static int_least32_t ice1000_swd_frequency(int_least32_t hz)
{
	if (hz > 0)
		ice1000_set_freq(hz);
...
static int_least32_t ice2000_swd_frequency(int_least32_t hz)
{
	if (hz > 0)
		ice2000_set_freq(hz);
...
static int ice1000_swd_queue_cmd(uint8_t cmd, uint32_t *dst, uint32_t data)
{
	uint8_t data_parity_trn[DIV_ROUND_UP(32 + 1, 8)];
	struct swd_packet *packet;
	int retval;

	retval = ice1000_swd_ensure_space(8 + 38);
	if (retval != ERROR_OK)
		return retval;
	...
	if (cmd & SWD_CMD_APnDP)
		retval = ice1000_swd_queue_idle_cycles(80);

	return retval;
}

static void ice1000_swd_write_reg(uint8_t cmd, uint32_t value, uint32_t ap_delay_hint)
{
	int retval;
	retval = ice1000_swd_queue_cmd(cmd, NULL, value);
	if (retval != ERROR_OK)
		LOG_ERROR("%s SWD write register failed", adi_cable_name());
}

static void ice1000_swd_read_reg(uint8_t cmd, uint32_t *value, uint32_t ap_delay_hint)
{
	int retval;
	retval = ice1000_swd_queue_cmd(cmd, value, 0);
	if (retval != ERROR_OK)
